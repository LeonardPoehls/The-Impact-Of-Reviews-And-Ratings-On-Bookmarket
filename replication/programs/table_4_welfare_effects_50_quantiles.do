...
use   tempfiles/quantity_us_simulations_50_quantiles.dta, clear
...
* calculate the NL parameters from the elasticities:
	...
	gen ALPHA = alpha*(1-sigma)/((1-sigma*s_jin-(1-sigma)*s)*p)
	reg ALPHA
	replace ALPHA = _b[_cons]

	gen GAMMA = -gamma*(1-sigma)/((1-sigma*s_jin-(1-sigma)*s)*R)
...
log using replication_logs/table_4_50_quantiles_log.txt, replace text 

*  50 quantiles CS result:

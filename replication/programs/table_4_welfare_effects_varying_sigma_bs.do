*********************************************************
** BS standard errors for varying sigma welfare calculation  
********************************************************* 	
***************************************************
...
set obs 1
gen x=.
save  tempfiles/welfare_effects_varying_sigma_bs.dta, replace 
...
forvalues sigma=0.0(0.95) 1.01 {
	...
	** keep totals & save for each sigma
	...
	append using tempfiles/welfare_effects_varying_sigma_bs.dta
	save tempfiles/welfare_effects_varying_sigma_bs.dta, replace 
...
use  tempfiles/welfare_effects_varying_sigma_bs.dta, clear  
	drop if sigma==.
	
************************************
* scale the stars estimate (but not the reviews estimate)
	...
	** Scale up to get total CS effects for stars
	gen dCS_stars_adj 	= dCS_stars*infl
...
************************************
	foreach i in dCS_stars_adj dCS_reviews ratio {
		qui: su `i' if sigma==0
		gen `i'_sd_0 = r(sd)
		qui: su `i' if sigma>0.94
		gen `i'_sd_95 = r(sd)
	}

log using replication_logs/table_4_varying_sigma_bs_log.txt, replace text

*  CS result for sigma=0 (standard errors):
	fsum *_sd_0 

*  CS result for sigma=0.95 (standard errors):
	fsum *_sd_95 

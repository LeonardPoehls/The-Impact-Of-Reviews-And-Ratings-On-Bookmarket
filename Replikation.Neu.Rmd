...
dataAll <- data %>%
  filter(ddate >= "2018-01-01" & ddate <= "2018-12-31") %>%
  arrange(canum, ddate) 
...
regTest <- feols(lrank ~ L1.rank + lpamzn + lreview + lR | canum, vcov = "hetero", data = dataUS)
...
regTest$residuals <- regTest$residuals - mean(regTest$residuals[which(dataUS$NYT_elapse == -1)])
 ...
 ggplot(reg1_agg, aes(x = NYT_elapse, y = Avg.Sales.rank)) + 
   geom_line() + 
...
###OTH Plot
 ...
 ggplot(reg2_agg, aes(x = OTH_elapse, y = Avg.Sales.rank)) + geom_line() + scale_x_continuous(breaks = seq(-20, 40, by = 10), limits = c(-20, 40)) + geom_line(aes(y = Max95), linetype = "dashed") + geom_line(aes(y = Min95), linetype = "dotted") +    scale_y_reverse() + 
   geom_vline(xintercept = -1, color = "red", size = 0.5) +
 ylim(0.3, -0.5) +
 theme_minimal() 
...
#Regression 1
dataAll <- dataAll %>%
  mutate(lRev_50 = ntile(lreview, 50))

regRev <- lm(lreview ~ lrR_50, data = dataAll)

regRev_agg <- aggregate(dataAll$lreview, by = list(dataAll$lrR_50), FUN = mean) 
 colnames(regRev_agg) <- c("lRev_50", "log.review")
 
dataAll <- merge(dataAll, regRev_agg, by = "lRev_50") 
 


#Regression 2
reg6 <- feols(lrank ~ L1.rank + lpamzn + lreview + log.review*lR + dnytpost1_1 + dnytpost1_2 + dnytpost1_3 + dnytpost6_1 + dnytpost6_2 + dnytpost6_3  + dnytpost10_1 + dnytpost10_2 + dnytpost10_3 + dothpost_1 + dothpost_2 + dothpost_3 + dnytpostpre_1 + dnytpostpre_2 + dnytpostpre_3 + dothpostpre_1 + dothpostpre_2 + dothpostpre_3 + dnytpostprer_1  + dnytpostprer_2  + dnytpostprer_3  + dothpost10_1  + dothpost10_2  + dothpost10_3  + dnytpost1r_3 + dnytpost1r_2 + dnytpost1r_3 + dnytpost6r_1 + dnytpost6r_2 + dnytpost6r_3 + dnytpost10r_1 + dnytpost10r_2 + dnytpost10r_3 + epos + epos2 + epos3 + eneg + eneg2 + eneg3 | canum, vcov = "hetero", data = dataAll)
...
#	estimates save "estimates/review_quantiles_50", replace
 ...
 reg3_agg <- aggregate(reg6$residuals, by = list(dataAll$log.review), FUN = mean) 
 colnames(reg3_agg) <- c("Quantiles", "log.Review.rank.effect")
 
# reg3_agg <- aggregate(reg6$residuals, by = list(dataAll$lrR_50), FUN = mean) 
# colnames(reg3_agg) <- c("Quantiles", "log.Review.rank.effect")
